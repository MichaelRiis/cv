...
Born: 22nd of July 1986\\
\\
Address: 2200 Copenhagen N, Denmark\\
\\
E-mail: michael.riis@gmail.com
%}
%&
...
Machine learning, probabilistic modelling, Bayesian statistics, approximate inference, Gaussian processes, sparse modelling, inverse problems, signal processing, neuroimaging, \& applied mathematics.
...
\item[] \textit{Ph.D., Machine learning and probabilistic modelling}%
        \hfill \textbf{April 2017 (expected)}
        \begin{innerlist}
        \item Sparse models for neuroimaging applications 
        \item Project title: \emph{Active Bayesian Sensing}
        \end{innerlist}
\end{outerlist}
...
\item[] \textit{Visiting student researcher}%
            \hfill \textbf{September 2015 - February 2016}
            \begin{innerlist}
                \item Visited Poldrack Lab for 5 months and worked on a project called "Model-based dynamic functional connectivity"
                \item Subjects: fMRI data analysis, resting state networks, sparsity, Gaussian processes, variational inference
            \end{innerlist}
...
\begin{outerlist}
    ...
    \item[] \textit{Student programmer}%
            \hfill \textbf{July 2013}
            \begin{innerlist}
                \item Real-time EEG preprocessing and automatic artifact removal using
...
\begin{outerlist}
	\item[]  \textit{Research assistant}%
			 \hfill \textbf{September 2011 to January 2012}
			 \begin{innerlist}
			 	\item Design, implementation and test of real-time computer vision algorithms for project PigWise
				\item Microsoft Kinect, image processing, machine learning, Matlab, C++, Qt
			 \end{innerlist}
...
\item[] \textit{Master thesis project co-supervisor}%
            \hfill \textbf{September 2016 - January 2017}
            \begin{innerlist}
                \item Co-supervised a project called "Real-time EEG analysis on a smart-phone"
                \item Subjects: Probabilistic modelling, linear inverse problems, sparsity, expectation propagation
            \end{innerlist}

\item[] \textit{Project co-supervisor}%
            \hfill \textbf{February 2016 - July 2016}
            \begin{innerlist}
                \item Co-supervised 3 groups of master students doing projects as a part of the course "Advanced machine learning"
                \item One project lead to a submission to the 42nd IEEE International Conference on Acoustics, Speech and Signal Processing (ICASSP 2017)
                \item Subjects: Machine learning, EEG source reconstruction, EEG decoding
            \end{innerlist}

\item[] \textit{Project supervisor}%
            \hfill \textbf{April 2015}
            \begin{innerlist}
                \item Supervised 5 groups of bachelor students doing projects as a part of a linear algebra \& calculus course
                \item Subjects: Linear and non-linear systems of equations, linear algebra, Taylor series and numerical optimization
            \end{innerlist}

\item[] \textit{Assistant teacher}%
            \hfill \textbf{September 2014 to December 2014}
            \begin{innerlist}
                \item TA in course ''Non-linear signal processing''
                \item Subjects: Machine learning based on Christopher Bishop's book
            \end{innerlist}

    \item[] \textit{Assistant teacher}%
            \hfill \textbf{September 2012 to June 2013}
            \begin{innerlist}
                \item Autumn semester 2012  and spring semester 2013 in the course {Mathematics 1} 
                \item Subjects: Linear algebra, single \& and multivariate calculus, Maple
            \end{innerlist}
\end{outerlist}
\halfblankline
...
\begin{outerlist}
    \item[]  \textit{Assistant teacher}%
             \hfill \textbf{June 2010 to december 2011}
             \begin{innerlist}
...
\item M. R. Andersen, S. T. Hansen, L. K. Hansen, "Learning The Solution Sparsity Of An Ill-Posed Linear Inverse Problem With The Variational Garrote", Machine learning for signalprocessing 2013 (Southampton, UK), 23rd of September 2013
\end{outerlist}
\halfblankline
...
\begin{bibenum}
\item M. R. Andersen,  O. Winther, L. K. Hansen, "Spatio-temporal spike and slab priors for MMV problems", extended abstract, Signal Processing with Adaptive Sparse Structured Representations (SPARS) 2015 (Cambrigde, UK), 6pp

\item M. R. Andersen, O. Winther, L. K. Hansen, ''Bayesian inference
for structured spike and slab priors'', conference paper, Advances in Neural Information
Processing Systems 2014 (Montreal, Canada), 9pp.

\item M. R. Andersen, S. T. Hansen, L. K. Hansen, "Learning the solution sparsity of an ill-posed linear inverse
problem with the variational garrote", conference paper, Machine Learning for Signal Processing 2013 (Southampton, UK), 6pp

\item T. Gregersen, T. Jensen, M.R. Andersen, L. Mortensen, J. Maselyne, E. Hessel, P. Ahrendt, "Consumer Grade range cameras for monitoring pig feeding behaviour", conference paper, European conference on precision livestock farming 2013 (Belgium), 10.-12. September 2013, 8pp.

\item T. Gregersen, T. Jensen, M.R. Andersen, L. Mortensen, J. Maselyne, E. Hessel, "Computer vision based
monitoring of performance of an RFID based eating registration system", conference paper, Construction, engineering and environment in livestock 2013 (Germany), 24.-26. September 2013, 6pp
\end{bibenum}
\halfblankline
...
\begin{bibenum}
	\item M.R. Andersen, T. Jensen, P. Lisouski, A.K. Mortensen, M.K. Hansen, T. Gregersen, P. Ahrendt, "Kinect depth sensor evaluation for computer vision algorithms", Aarhus University, Department of Engineering, February
2012
...
\begin{bibenum}
    \item M. R. Andersen, A. Vehtari, O. Winther, L. K. Hansen, "Bayesian inference for spatio-temporal spike and slab priors"
